...
/**
 * check_sub_tree_Left - Check if the binary tree is a valid left subtree.
 *
...
/**
 * check_sub_tree_Right - Check if the binary tree is a valid right subtree.
 *
 * @node: Pointer to the root of the subtree to be checked.
...
int binary_tree_is_bst(const binary_tree_t *tree)
{
	...
	if (tree->left && tree->left->n < tree->n)
	{
		valid_subtree = check_sub_tree_Left(tree->left, tree->n);
		if (valid_subtree == 0)
			return (0);
	...
	if (tree->right && tree->right->n > tree->n)
	{
		valid_subtree = check_sub_tree_Right(tree->right, tree->n);
		if (valid_subtree == 0)
			return (0);
